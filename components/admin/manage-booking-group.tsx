...
export function ManageBookingGroup({
  ...
  const [taFilter, setTaFilter] = useState<"all" | "complete" | "incomplete">("all")
  ...
  const [studentBookingFilter, setStudentBookingFilter] = useState<"all" | "booked" | "not_booked">("all")
  ...
  const handleStatusChange = async (newStatus: string) => {
    const supabase = createClient()
    const { error } = await supabase.from("booking_groups").update({ status: newStatus }).eq("id", bookingGroup.id)
  ...
  const filteredSlots = useMemo(() => {
    return slots.filter((slot) => {
      ...
      const matchesDate = slotDateFilter === "all" || new Date(slot.start_time).toLocaleDateString() === slotDateFilter
      return matchesTA && matchesDate
    })
  }, [slots, slotTaFilter, slotDateFilter])

  const upcomingSlots = filteredSlots.filter((slot) => new Date(slot.start_time) > new Date())
  const pastSlots = filteredSlots.filter((slot) => new Date(slot.start_time) <= new Date())
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter(
      (booking) =>
        booking.student?.name?.toLowerCase().includes(search) || booking.student?.email?.toLowerCase().includes(search),
    )
  ...
  const completeTAs = taStats.filter((ta) => ta.total_minutes >= ta.required_minutes).length
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-5">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-2xl font-bold text-emerald-600">{completeTAs}</div>
              <p className="text-sm text-muted-foreground">TAs Complete</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
              <div className="text-2xl font-bold text-amber-600">{incompleteTAs}</div>
              <p className="text-sm text-muted-foreground">TAs Incomplete</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
              <div className="text-2xl font-bold text-rose-600">{studentsNotBooked}</div>
              <p className="text-sm text-muted-foreground">Students Not Booked</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="slots" className="space-y-6">
          ...
          <TabsContent value="slots">
              ...
              <CardHeader>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Select value={slotTaFilter} onValueChange={setSlotTaFilter}>
                      <SelectTrigger className="w-40">
                    ...
                    <Select value={slotDateFilter} onValueChange={setSlotDateFilter}>
                      <SelectTrigger className="w-40">
              ...
              <CardContent>
                {upcomingSlots.length === 0 ? (
                  <div className="py-8 text-center">
                    ...
                    <p className="mt-4 text-muted-foreground">No slots match your filters</p>
                    <Button variant="outline" className="mt-4 bg-transparent" onClick={() => setShowCreateSlot(true)}>
                      Add a slot
                    </Button>
                  </div>
                ) : (
                    ...
                    {upcomingSlots.map((slot) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant={isFull ? "secondary" : "default"}>{isFull ? "Full" : "Available"}</Badge>
            ...
            {pastSlots.length > 0 && (
              <Card className="mt-6">
                <CardHeader>
                  <CardTitle className="text-muted-foreground">Past Slots ({pastSlots.length})</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <CardHeader>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setStudentBookingFilter(v as "all" | "booked" | "not_booked")}
                  >
              ...
              <CardContent>
                {filteredStudents.length === 0 ? (
                  <div className="py-8 text-center">
                    ...
                    <p className="mt-4 text-muted-foreground">No students match this filter</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {filteredStudents.map((student) => {
                      const hasBooked = bookedStudentIds.has(student.id)
                      const studentBooking = bookings.find((b) => b.student_id === student.id)
                      return (
                        <div
                          key={student.id}
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Select value={taFilter} onValueChange={(v) => setTaFilter(v as "all" | "complete" | "incomplete")}>
                    <SelectTrigger className="w-40">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <CardContent>
                {filteredTAs.length === 0 ? (
                  <div className="py-8 text-center">
                    ...
                    <p className="mt-4 text-muted-foreground">No TAs match this filter</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredTAs.map((ta) => {
                      const progress =
                        ta.required_minutes > 0 ? Math.min(100, (ta.total_minutes / ta.required_minutes) * 100) : 100
                      const isComplete = ta.total_minutes >= ta.required_minutes
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              <p className="font-medium">
                                {Math.round(ta.total_minutes)} / {ta.required_minutes} min
                              </p>
                              <p className="text-sm text-muted-foreground">{ta.slots_count} slots</p>
                            </div>
                          </div>
      ...
      <CreateSlotDialog open={showCreateSlot} onOpenChange={setShowCreateSlot} bookingGroupId={bookingGroup.id} />
    </div>
  )
}
